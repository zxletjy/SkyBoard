...
#define MPU6050_ADDRESS         0xD2	// 0x69
...
u8 Acc_CALIBRATED;
u8 Gyro_CALIBRATED;
Vector3i Acc_Offset = {0,0,0},Gyro_Offset = {0,0,0};
Vector3f Acc_ADC = {0,0,0},Gyro_ADC = {0,0,0};
Vector3f Gyro_dps = {0,0,0};
...
void Read_Gyro_Data(void)
{
	int16_t gyro_temp[3];
	u8 mpu6050_buffer[6]; //接收数据缓存区
	mpu6050_buffer[0] = I2C_Single_Read(MPU6050_ADDRESS,MPU_RA_GYRO_XOUT_L); 
	mpu6050_buffer[1] = I2C_Single_Read(MPU6050_ADDRESS,MPU_RA_GYRO_XOUT_H);
	gyro_temp[1] = -((((int16_t)mpu6050_buffer[1]) << 8) | mpu6050_buffer[0])- Gyro_Offset.y;	//陀螺仪X轴

	mpu6050_buffer[2] = I2C_Single_Read(MPU6050_ADDRESS,MPU_RA_GYRO_YOUT_L);
	mpu6050_buffer[3] = I2C_Single_Read(MPU6050_ADDRESS,MPU_RA_GYRO_YOUT_H);
	gyro_temp[0] = ((((int16_t)mpu6050_buffer[3]) << 8) | mpu6050_buffer[2])- Gyro_Offset.x;	//陀螺仪Y轴

	mpu6050_buffer[4] = I2C_Single_Read(MPU6050_ADDRESS,MPU_RA_GYRO_ZOUT_L);
	mpu6050_buffer[5] = I2C_Single_Read(MPU6050_ADDRESS,MPU_RA_GYRO_ZOUT_H);
	gyro_temp[2] = ((((int16_t)mpu6050_buffer[5]) << 8) | mpu6050_buffer[4])- Gyro_Offset.z;	  //陀螺仪Z轴		
...
void CalOffset_Gyro(void)
{
	if(Gyro_CALIBRATED)
	{
		static Vector3f	tempGyro = {0,0,0};
